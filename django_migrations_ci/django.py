...
def clone_test_db(parallel, is_pytest=False, database="default"):
    ...
    with _override_database_name(connection):
        for index in range(parallel):
            ...
            connection.creation.clone_test_db(
                suffix=suffix, verbosity=True, keepdb=False
            )
            ...
            if is_pytest and connection.vendor == "sqlite":
                ...
                if "." in django_db_name:
                    ...
                    pytest_db_name = re.sub(
                        r"(_gw\d+)\.(.+)$", r".\2\1", django_db_name
                    )
...
@contextmanager
def _override_database_name(connection):
    ...
    connection.close()
    settings.DATABASES[database]["NAME"] = database_name
